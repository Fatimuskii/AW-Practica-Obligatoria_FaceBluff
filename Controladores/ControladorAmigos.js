...
function mostrarPerfilAmistad(request, response, next){


    oModeloAmigos.buscarAmistad(request.params.id, function(err, usuario){
        if (err){
            next(err);

        }else{
            response.status(200);
            response.render("PaginaDePerfilAmistad", {usuario: usuario} ); 
        }
...
module.exports = {
    mostrarPerfilAmistad: mostrarPerfilAmistad,
